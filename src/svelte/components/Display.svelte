<script>
	...
	// Own headless setting applies to dockList, the classic row stack, grid and
	// split; other layouts manage their header visibility differently.
	let effectiveHeadless = $derived(headless || ([CST.LIST_LAYOUT_DOCK, CST.LIST_LAYOUT_STACK, CST.LIST_LAYOUT_GRID, CST.LIST_LAYOUT_SPLIT].includes(layout) && (type?.[CST.TYPE_HEADLESS] ?? false)));
